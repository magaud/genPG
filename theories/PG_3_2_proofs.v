...
Lemma points_line : forall T Z:Point, forall x:Line,
      incid_lp T x -> incid_lp Z x -> (eqP T Z)= false -> x = (l_from_points T Z).
Proof.
...
Ltac prove_a1_unique :=
  ...
  destruct A; destruct B; solve [discriminate |  
                                 handle l1;handle l2; exact (@erefl Line _)].

Lemma a1_unique:forall (A B :Point)(l1 l2:Line),
    eqP A B = false ->
    incid_lp A l1 -> incid_lp B l1 -> incid_lp A l2 -> incid_lp B l2 -> l1=l2.
Proof.
...
Ltac prove_uniqueness :=
  ...
  intros Hneq; handle l; handle m; rewrite Bool.orb_false_l; exact (erefl true)]].

Lemma uniqueness : forall (A B :Point)(l1 l2:Line),
    incid_lp A l1 -> incid_lp B l1  -> incid_lp A l2 -> incid_lp B l2 ->
    (eqP A B) || (eqL l1 l2).
Proof.
  (*intros P Q l m Hl Hl' Hm Hm';case_eq (eqP P Q);
    [ intros; apply orTb | 
  destruct P; destruct Q; try solve [discriminate |
  intros Hneq l Hl Hl' m Hm Hm'; handle l; handle m; 
  rewrite Bool.orb_false_l; exact (erefl true)]].*)
...
(*Ltac exists_lppp l t u v := exists l; exists t; exists u; exists v; unfold Intersect_In; simpl; apply erefl.*)
...
Proof.
  ...
  case v1;(*case v2; intros hp1p2; try apply (@degen_bool _ hp1p2) ; case v3;
            intros hp1p3 hdist x;(* unfold f_a3_3 in x; *)try (apply (@degen_bool _ hp1p3)); try (apply  (@degen_bool _ hdist));exists_lppp (fst x) (fst (fst (snd x))) (snd (fst (snd x))) (snd (s     nd x)).*)
...
Module PS : ProjectiveSpace.
  ...
  Lemma uniqueness : forall (A B :Point)(l1 l2:Line),
      incid_lp A l1 -> incid_lp B l1  -> incid_lp A l2 -> incid_lp B l2 -> eqP A B || eqL l1 l2.
  Proof.
    exact uniqueness.
  Qed.
